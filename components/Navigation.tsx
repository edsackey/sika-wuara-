...
import { LayoutDashboard, Languages, Landmark, MessageSquareText, CalendarClock, Users, Wallet } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({ activeTab, onTabChange }) => {
  const items = [
    ...
    { id: AppTab.ADVISOR, label: 'Advice', icon: MessageSquareText },
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 glass-morphism border-t border-slate-800 px-2 py-2 md:top-0 md:bottom-auto md:border-b md:border-t-0">
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        <div className="hidden md:flex items-center space-x-2 mr-8 shrink-0">
          ...
          <span className="font-bold text-lg tracking-tight">SIKA WURA <span className="text-amber-500">AI</span></span>
        </div>
        <div className="flex flex-1 justify-around md:justify-end md:space-x-6">
          {items.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => onTabChange(id)}
              className={`flex flex-col md:flex-row items-center space-y-1 md:space-y-0 md:space-x-2 p-2 rounded-xl transition-all ${
                activeTab === id 
